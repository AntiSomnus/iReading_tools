...
conn = pymysql.connect(host='127.0.0.1', port=3306, user='root', passwd='666.', db='ireading')

cur = conn.cursor()
cur.execute("select Max(id) from word_entire e where e.word REGEXP '^[A-Za-z]+$' and e.word not in (select m.word from word_slim m) and uk_pron is not null and us_pron is not null")
for item in cur:
    index = item[0] if item[0] else 0
cur.execute("select word from word_entire e where e.word REGEXP '^[A-Za-z]+$' and e.word not in (select m.word from word_slim m) and id>" + str(index))
l = []
c=0
count = index
for item in cur:
